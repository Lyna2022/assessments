...
okanagan.colour.ramps <- function(left.num,right.num) {
  
  color.brewer.yellow.green.blue <- c("#FFFF66", "#99FF33", "#00FF00", "#00CC66", "#009999","#003366")  
  yellow.green.blue <- colorRampPalette(colors=color.brewer.yellow.green.blue, bias=1, space = "Lab", interpolate = "linear")

  color.brewer.light.brown.to.dark.brown <- c("#F6E8C3","#DCB478","#C89150","#AF6932") 
  light.brown.to.dark.brown <- colorRampPalette(colors=color.brewer.light.brown.to.dark.brown, bias=1, space = "Lab", interpolate = "linear")

  if (length(right.num)==0 | length(left.num)==0) {
    if (length(right.num)==0)
      colour.ramp <- rev(light.brown.to.dark.brown(left.num))                 
    if (length(left.num)==0)
      colour.ramp <- yellow.green.blue(right.num)
  } else {
    colour.ramp <- c(rev(light.brown.to.dark.brown(left.num)),yellow.green.blue(right.num))
  }
...
reg.ds.maps <- function(data,box.data,region,
                        ...
                        overlays=NULL,
                        leg.loc='topright',width=1200,height=900,
  ...
  plot.window.xlim <- c((xlim.min - xlim.adj), (xlim.max))
  plot.window.ylim <- c((ylim.min-ylim.adj*0.1), (ylim.max + ylim.adj))
...
##  class.breaks <- c(0,seq(100,200,by=10),300,400,500,1000)
  ...
  plot(c(),xlim=plot.window.xlim,ylim=plot.window.ylim,xaxs='i',yaxs='i',
       bg='lightgray',mgp=c(3.5,2,0),axes=FALSE)
##       xlab='Longitude (\u00B0E)',ylab='Latitude (\u00B0N)',main=plot.title,    
  ...
  city.dir <- '/home/data/gis/basedata/BC_Cities/' 
  ...
  shape.dir <- '/home/data/projects/rci/data/moti/nrcan-precip_case_studies/moti_shapefiles/'
  ...
  lakes.shp <- readOGR('/home/data/projects/rci/data/assessments/okanagan/shapefiles/','okanagan_lakes',stringsAsFactors=F, verbose=F)
  ...
  lines(bc.roads,col='black',lwd=4)
  lines(bc.roads,col='yellow',lwd=3) 
  if (!is.null(overlays)) {
  ...
  shadowtext(cx+0.01,cy-0.01,'Kelowna',adj=4,pos=4,cex=2,col='black',bg='white',r=0.2)

  cx <- -120.336
  cy <- 50.674
  points(cx,cy,pch=19,cex=2.5,col='black',bg='black')
  shadowtext(cx+0.01,cy-0.01,'Kamloops',adj=4,pos=4,cex=2,col='black',bg='white',r=0.2)

  cx <- -119.269
  cy <- 50.264
  points(cx,cy,pch=19,cex=2.5,col='black',bg='black')
  shadowtext(cx+0.01,cy-0.01,'Vernon',adj=4,pos=4,cex=2,col='black',bg='white',r=0.2)
  
  cx <- -119.596
  cy <- 49.5012
  points(cx,cy,pch=19,cex=2.5,col='black',bg='black')
  shadowtext(cx+0.01,cy-0.01,'Penticton',adj=4,pos=4,cex=2,col='black',bg='white',r=0.2)

  cx <- -119.4461
  cy <- 49.0325
  points(cx,cy,pch=19,cex=2.5,col='black',bg='black')
  shadowtext(cx+0.01,cy+0.02,'Osoyoos',adj=4,pos=4,cex=2,col='black',bg='white',r=0.2)

  cx <- -119.1396
  cy <- 50.5508
  points(cx,cy,pch=19,cex=2.5,col='black',bg='black')
  shadowtext(cx+0.01,cy+0.02,'Enderby',adj=4,pos=4,cex=2,col='black',bg='white',r=0.2)

  
  ##Regional Districts
  cx <- -120.5
  cy <- 49.4
  shadowtext(cx+0.01,cy-0.01,'Okanagan-Similkameen',adj=4,pos=4,cex=2.5,col='black',bg='white',r=0.2)
  cx <- -120.25
  cy <- 50.0
  shadowtext(cx+0.01,cy-0.01,'Central Okanagan',adj=4,pos=4,cex=2.5,col='black',bg='white',r=0.2)
  cx <- -119.0
  cy <- 50.5
  shadowtext(cx+0.01,cy-0.01,'North Okanagan',adj=4,pos=4,cex=2.5,col='black',bg='white',r=0.2)
