...
reg.ds.maps <- function(data,box.data,region,
  ...
  ##Set plot boundaries
  xlim.adj <- (xlim.max - xlim.min) * 0.15 ##0.025
  ylim.adj <- (ylim.max - ylim.min) * 0.15 ##0.025 
...
##  class.breaks <- c(0,seq(100,200,by=10),300,400,500,1000)
  ...
  ##city.dir <- '/home/data/gis/basedata/BC_Cities/' ##'/home/data/projects/rci/data/forestry/regional_summaries/shapefiles/'
  nanaimo.dir <- '/home/data/projects/rci/data/assessments/nanaimo/shapefiles/'
  nanaimo.rd <- spTransform(get.region.shape('nanaimo_roads',nanaimo.dir),CRS(proj)) ## mjrcities2
  bc.cities <- spTransform(get.region.shape('nanaimo_roads',nanaimo.dir),CRS(proj)) ## mjrcities2
  ...
  lakes.shp <- readOGR('/home/data/projects/rci/data/assessments/nanaimo/shapefiles/','nanaimo_lakes',stringsAsFactors=F, verbose=F)
  ...
  lines(nanaimo.rd,col='darkgray',lwd=2.5) 
