...
get.region.names <- function(region) {
  return(list(area='van_test',subset='van_test',region='van_test'))
}
...
get.crop.box <- function(region) {
  return(c(-123.0,-120.0,49.0,51.0))
}
...
add.plot.overlays <- function(crs) {
  shape.dir <- '/storage/data/projects/rci/data/assessments/shapefiles/van_test/'
  region.shp <- readOGR(shape.dir,'van_test',stringsAsFactors=F, verbose=F)
