...
make.raster.version <- function(nc,data) {
  ...
  ts <- rasterize(spatial.coords,rs,aperm(data,c(2,1)),fun=mean)
...
##  pr.max <- array(0,c(nc$dim$y$len,nc$dim$x$len,ylen))
##  pr.mean <- array(0,c(nc$dim$y$len,nc$dim$x$len,ylen))
##  pr.total <- array(0,c(nc$dim$y$len,nc$dim$x$len,ylen))

if (1==0) {
  for (i in 1:ylen) {
  print(paste0('Year: ',levels(yrs)[i]))
    ix <- grep(yrs[i],series)
    data <- ncvar_get(nc, varid=var.name,start=c(1,1,1,ix[1]),count=c(-1,-1,-1,length(ix)))

    pr.data <- data/8 ##To address the incorrect conversion from pr flux
    dy.fac <- as.factor(format(series[ix],'%Y-%m-%d'))
    pr.daily <- aperm(apply(pr.data,c(1,2),function(x,y){tapply(x,y,sum)},dy.fac),c(2,3,1)) ##Convert to daily total
    pr.max[,,i] <- apply(pr.daily,c(1,2),max)
##    pr.mean[,,i] <- apply(pr.daily,c(1,2),mean)    
    pr.total[,,i] <- apply(pr.daily,c(1,2),sum)
  }
  pr.max.avg <- apply(pr.max,c(1,2),mean)
  pr.total.avg <- apply(pr.total,c(1,2),mean)
}

data <- ncvar_get(nc, varid=var.name,start=c(1,1,1,1),count=c(-1,-1,-1,31))
pr.total.avg <- apply(data,c(1,2),mean)

print(range(pr.total.avg))
print(mean(pr.total.avg))

  plot.dir <- '/storage/data/projects/rci/data/RCM/CRCM5/plots/'

##  make.crcm5.plot(pr.max.avg,nc,var.name,
##                  plot.title='PR MAX',plot.file=paste0(plot.dir,'era.pr.max.png'))
##  make.crcm5.plot(pr.mean.avg,nc,var.name,
##                  plot.title='PR MEAN',plot.file=paste0(plot.dir,'era.pr.mean.png'))
  make.crcm5.plot(pr.total.avg,nc,var.name,
                  plot.title='TX AVG',plot.file=paste0(plot.dir,'era.tx.existing.png'))
...
nc <- nc_open(rcm.file)
##test <- annual.averages(nc,var.name='T9')

###read.crcm5.data(nc,'PR')
