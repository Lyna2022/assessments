...
compute.climate.values <- function(model,ds.type,
  ...
  ##GCM and Regional averages grouping monthly and seasonal values - needs pre-computed seasonal and monthly files
    ...
    seas.files <-  seas.files[grep('standard_deviation_seasonal',seas.files)]
    ...
    ann.files <-  ann.files[grep('standard_deviation_annual',ann.files)]
    ...
    ##Function to extract subset of data for moti region
    past.values <- c(past.seas.avg,past.ann.avg) ##c(past.mon.avg,past.seas.avg,past.ann.avg)
    proj.values <- c(proj.seas.avg,proj.ann.avg) ##c(proj.mon.avg,proj.seas.avg,proj.ann.avg)
...
format.tables <- function(mon.vals,models,rd,pctl=FALSE,var.name,region.title) {
  ...
  if (var.name=='snowdepth') {
    ...
    title <- c(paste('Table: ',get.variable.title(var.name),' for ',region.title,sep=''),rep(' ',5))
  }
...
make.tables <- function(model.list,
  ...
  var.list <- c('tas') ##,'tas','tasmax','tasmin')  
  ##Climate parameters
  for (var.name in var.list) {
    ...
    my.writedir <- paste(write.dir,'tables/build_code/',var.name,'/',sep='')
    ...
    write.table(past.table,file=paste(my.writedir,'past.',var.name,'.standard_deviation.',scenario,'.',past.int,'.csv',sep=''),sep=',',quote=FALSE,col.name=FALSE,row.name=FALSE)
    if (1==0) {
      ...
      write.table(future.table,file=paste(my.writedir,'future.',var.name,'.standard_deviation.',scenario,'.',proj.int,'.csv',sep=''),sep=',',quote=FALSE,col.name=FALSE,row.name=FALSE)
      ...
      write.table(abs.table,file=paste(my.writedir,'abs.anomalies.',var.name,'.standard_deviation.',scenario,'.',proj.int,'.csv',sep=''),sep=',',quote=FALSE,col.name=FALSE,row.name=FALSE)
      ...
      write.table(prc.table,file=paste(my.writedir,'percent.anomalies.',var.name,'.standard_deviation.',scenario,'.',proj.int,'.csv',sep=''),
